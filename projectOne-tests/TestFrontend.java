...
public class TestFrontend {
	...
	public void runTests() {
		...
		System.out.print("Test 'r' load rating selection screen (WARNING: if pressing 'x' twice from the genre selection screen does not exit app, test won't exit and run indefinitely!): ");
		if (this.testFrontendRForRatings()) {
			System.out.println("PASSED");
		} else {
			System.out.println("FAILED");
		}
		System.out.print("Test 'r' load rating selection screen and select one rating (WARNING: if pressing 'x' twice from the genre selection screen does not exit app, test won't exit and run indefinitely!): ");
		if (this.testFrontendOneRatingSelected()) {
			System.out.println("PASSED");
		} else {
			System.out.println("FAILED");
		}
	 ...
	 * @return true if the test passed, false if it failed
	 */
	public boolean testFrontendOneRatingSelected() {
		PrintStream standardOut = System.out;
		InputStream standardIn = System.in;
		try {
			// set the input stream to our input (with an g to test of the program lists genres)
			String input = "r" + System.lineSeparator() + "3" + System.lineSeparator() + "x" + System.lineSeparator() + "x";
			InputStream inputStreamSimulator = new ByteArrayInputStream(input.getBytes());
			System.setIn(inputStreamSimulator);
			ByteArrayOutputStream outputStreamCaptor = new ByteArrayOutputStream();
			// set the output to the stream captor to read the output of the front end
			...
			if (frontend != null && appOutput.contains("0")
					...
					&& appOutput.contains("9")
					&& appOutput.contains("10")
                    && appOutput.contains("The Source of Shadows")) {
                /**
